  ...
  async create({ avatar, userId }: ImageProps): Promise<Image> {
    try {
      ...
      if(err instanceof PrismaClientKnownRequestError){
        switch(err.code){
          case 'P2002': {
            throw new AppError('There is a unique constraint violation, a new user cannot be created with this email', 500);
          }
          case 'P2003': {
            throw new AppError('Foreign key constraint failed',500)
          }
          case 'P2005':{
            throw new AppError('The value stored in the database for the field is invalid for the field\'s type', 500)
          }
          default:{
    ...
    try {
      ...
      if(err instanceof PrismaClientKnownRequestError){
        switch(err.code){
          ...
          case 'P2025': {
            throw new AppError("An operation failed because it depends on one or more records that were required but not found. {cause}",500)
          }
          case 'P2005':{
            throw new AppError('The value stored in the database for the field is invalid for the field\'s type', 500)
          }
  ...
  async delete(id: string): Promise<void> {
    try {
      ...
      if(err instanceof PrismaClientKnownRequestError){
        switch(err.code){
          case 'P2027':{
            throw new AppError("Multiple errors occurred on the database during query execution", 500)
          }
          case 'P2019':{
